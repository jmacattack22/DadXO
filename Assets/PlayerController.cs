...
public class PlayerController : MonoBehaviour
{
    ...
    {
        if (anim.GetCurrentAnimatorStateInfo(0).IsName("PixelCharAnim_Sword_quickAtk") && col.gameObject.tag == "Enemy")
        {
            Debug.Log("Hit");
            if (facingRight)
            {
                col.gameObject.GetComponent<Rigidbody2D>().AddForce(Vector2.right * moveForce);
            }
            else
            {
                col.gameObject.GetComponent<Rigidbody2D>().AddForce(Vector2.left * moveForce);
            }        
        }
    }
}       